...
- **Frontend**: Start the frontend development server with `npm run start:web`.
- **Mobile App**: For Android, use `nx start mobile`, and for iOS, use `react-native run-ios`.
...
- **Frontend**: Indítsa el a frontend fejlesztői szerverét a `npm run start:web` paranccsal.
- **Mobilalkalmazás**: Android esetén használja a `nx start mobile` parancsot, iOS esetén pedig a `react-native run-ios` parancsot.
- **Webes Admin Felület**: A webes admin felületet egy webböngészőn keresztül érheti el.